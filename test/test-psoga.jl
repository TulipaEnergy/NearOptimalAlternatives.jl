@testset "Test generate alternatives using PSOGA." begin
  ...
  @testset "Test regular run with three alternatives." begin
    ...
    results = NearOptimalAlternatives.generate_alternatives(model, 0.1, 3, algorithm)
    ...
    @test length(results.solutions) == 3 &&
          ...
          (results.objective_values[2] ≥ 1.8 || isapprox(results.objective_values[2], 1.8)) &&
          (results.objective_values[2] ≤ 2.0 || isapprox(results.objective_values[2], 2.0)) &&
          (results.objective_values[3] ≥ 1.8 || isapprox(results.objective_values[3], 1.8)) &&
          (results.objective_values[3] ≤ 2.0 || isapprox(results.objective_values[3], 2.0))
  end
  ...
  @testset "Test regular run with two alternatives and maximise minimum distance." begin
    ...
    algorithm = NearOptimalAlternatives.PSOGA(N_solutions = 2, N = 200, maximise_total = false)
